...
- **UDA scelte dal Dipartimento** — catalogo autonomo e modificabile delle decisioni del 9 settembre 2026: due UDA adottate nel biennio, tre percorsi FSL e tre scelte d'asse nel triennio, più due simulazioni della seconda prova in quinta. Le dieci UDA complete sono memorizzate nella sezione con identificativi propri; la scelta d'asse della terza è una sola UDA, riscritta il 21 settembre 2026 sugli adolescenti — prevenzione, tutela, diritti e servizi — perché la versione centrata sulla prima infanzia ripeteva la scelta FSL della stessa classe. [Registro delle modifiche del 21 settembre](revisioni/2026-09-21-modifiche-dipartimento/REGISTRO.md).
- **UDA d'asse** — il catalogo d'asse del curricolo, una sola sezione e un solo file: 36 unità per il quinquennio. Ventuno nascono dall'accorpamento delle 42 schede del fascicolo di origine che un unico compito di realtà mobilita insieme e ne portano due competenze in uscita; sei restano autonome dal riordino; nove sono le proposte dei docenti, otto dagli ODT della collega e la 5.13 concordata il 7 settembre 2026. Ogni scheda dichiara le schede di origine da cui nasce. Le ore delle unità da accorpamento sono la somma delle origini, non una durata già validata. Dati generati da `tools/genera_uda_asse.py` con `tools/revisione_uda_asse.json`. Le indicazioni della Prof.ssa Manca, le attribuzioni integrative di Metodologie operative e i relativi effetti sulle ore sono documentati nel [registro del 7 settembre](revisioni/2026-09-07-indicazioni-manca/REGISTRO.md); accorpamenti, motivazioni e controlli nel [registro del 6 settembre](revisioni/2026-09-06-riordino-applicato/REGISTRO.md).
...
- **UDA Educazione civica** — catalogo autonomo collegato ai nuclei e alle competenze nazionali delle Linee guida allegate al D.M. 183/2024. La scheda di prima copre, come proposta da deliberare, le 33 ore annue minime previste dalla L. 92/2019; le 33 ore non sono presentate come obbligo per ogni UDA.
  Le modifiche del Dipartimento del 9 settembre 2026 sono documentate nel [registro dedicato](revisioni/2026-09-09-modifiche-dipartimento/REGISTRO.md).
- **Ripartizione oraria** — in ogni scheda UDA le ore sono inizialmente divise fra gli insegnamenti coinvolti in proporzione al quadro orario dell'istituto e compaiono accanto a ciascuna materia; l'intestazione indica la durata minima in settimane. A docente autenticato la distribuzione è liberamente modificabile e il sito segnala, senza bloccare il salvataggio, l'eventuale differenza rispetto al monte ore indicativo.
...
- **Navigazione** — le pagine aperte da sole hanno in alto una barra fissa con il ritorno al curricolo e le sezioni disponibili; nella pagina principale, quando le linguette escono dallo schermo, compare in basso una barra rapida per cambiare sezione. Gli indirizzi `index.html#dipartimento`, `#asse`, `#trasversali`, `#civica`, `#fsl`, `#esame`, `#didattica`, `#normativa` e simili aprono direttamente la sezione.
...
- **Piano delle UDA** — pagina del coordinatore: in evidenza le UDA scelte dal Dipartimento, sotto gli altri cataloghi come riserva fuori delibera, periodi, bozza nel browser, controllo del doppio conteggio ed esportazione Word/PDF in formato sintetico o integrale. La compilazione non attesta una delibera.
- **PFI** — progetto formativo individuale compilabile con scelta delle UDA dai quattro cataloghi pubblicati: d'asse, trasversali, Educazione civica e FSL.
...
Nei quattro cataloghi UDA sono disponibili:
...
- `uda-asse.html` + `uda-asse.js` + `uda-asse.css` + `data-uda-asse.json` — UDA d'asse del curricolo, catalogo unico generato da `tools/genera_uda_asse.py`
- `uda.html` + `uda.js` + `data-uda.json` — fascicolo di origine delle 57 schede d'asse: materiale storico, fuori dalla navigazione, sorgente del generatore e documentazione della provenienza
- `uda-dipartimento.html` + `uda-dipartimento.js` + `uda-dipartimento.css` + `data-uda-dipartimento.json` — catalogo autonomo con le copie complete delle UDA scelte e delle due simulazioni previste per la quinta
- `tools/derivazione_dipartimento.json` — da dove viene ogni UDA scelta dal Dipartimento, in un posto solo: lo leggono `tools/allinea_uda_dipartimento.py`, che riallinea le copie ai cataloghi, `tools/verifica_uda_scelte_dipartimento.cjs`, che controlla che siano allineate, e il Piano delle UDA, che senza quello non saprebbe che l'unità adottata e quella del catalogo sono la stessa
- `assets/uda-stampa.js` + `assets/uda-stampa.css` — stampa della singola UDA in Word e PDF, dentro la scheda aperta: catalogo d'asse, trasversali, Educazione civica, FSL, scelte del Dipartimento ed Esame di Stato. Le schede per la prova professionale hanno un documento proprio, sul quadro di riferimento della seconda prova invece che sul format del Box n. 8
- `uda-trasversali.html` + `uda-trasversali.js` + `data-uda-trasversali.json` — UDA trasversali
- `uda-civica.html` + `uda-civica.js` + `data-uda-civica.json` — UDA di Educazione civica, con competenza nazionale, obiettivo e monte annuo proposto
...
- `assets/uda-revisione.js` + `assets/uda-revisione.css` — modifica e revisione collegiale dei quattro cataloghi; saperi documentali protetti e integrazioni libere
- `supabase/consenti-uda-dipartimento.sql` — migrazione che abilita le chiavi autonome del catalogo dipartimentale
