...
(async()=>{fs.mkdirSync(out,{recursive:true});const b=await chromium.launch({headless:true,executablePath:process.env.PLAYWRIGHT_CHROMIUM_EXECUTABLE});try{
 ...
 assert.deepEqual(errors,[]);console.log('PASS UI locale: U1.1/U2.1 distinte in seconda; distribuzione 6/9; creazione di due idee; rilettura; modifica; errore senza perdita testo; escaping; altro docente legge senza modificare; mobile; navigazione; progettazione annuale vuota. API simulata, nessuna scrittura live.');
