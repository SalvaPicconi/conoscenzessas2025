...
for(const [file,tab,count] of [['uda.html','uda',57],['uda-trasversali.html','trasversali',14],['uda-fsl.html','fsl',4]]){
 ...
 for(const check of [f.locator('.stampa-scelta input').first(),f.locator('.stampa-scelta input').last()]) await azione(p,check,'check');
 ...
 await azione(p,f.locator('#stampa-selezionate'));assert.equal(await p.locator('.stampa-documento .uda-acc').count(),2);
 assert.deepEqual(await p.locator('.stampa-documento .uda-acc').evaluateAll(es=>es.map(e=>e.dataset.id)),[ids[0],ids.at(-1)]);
 await p.pdf({path:out+'/'+tab+'-selezionate.pdf',preferCSSPageSize:true});
 await azione(p,f.getByRole('button',{name:'Stampa piano UDA',exact:true}));assert.equal(await p.locator('.stampa-piano > tbody > tr').count(),2);
