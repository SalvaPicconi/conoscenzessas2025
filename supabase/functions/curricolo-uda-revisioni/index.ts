...
const FIELDS = new Set([
  ...
  "traccia", "personalizzazione", "descrizione", "statoDecisione",
]);
...
async function upsertRevision(request: Request, payload: Record<string, unknown>, sessionAuthor: string) {
  try {
    ...
    if (!/^([0-9]+\.[0-9]+|U[1-5]\.[0-9]+[a-z]?|T[1-5]\.[0-9]+|FSL[3-5]\.[0-9]+|E[3-5]\.[0-9]+|DIP(1-CIVICA|2-TRASVERSALE|[3-5]-(FSL|ASSE))|SIM5-[12]|nuova-(t-|f-)?[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})$/i.test(udaKey)) {
    ...
    if (!permissions.manage_status && ["approvata", "applicata"].includes(String(record.stato))) {
      throw new Error("Operazione non consentita: approvazione riservata alla gestione.");
    }
...
Deno.serve(async (request: Request) => {
  ...
  if (["vote", "ballot", "choice", "rate"].includes(action)) return json(request, { error: "Votazione temporaneamente non disponibile." }, 423);
